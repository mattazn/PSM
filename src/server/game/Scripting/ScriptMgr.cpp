            ...
            if (script->IsDatabaseBound())
            {
                ...
                if (id)
                {
                    ...
                    else
                    {
                        ...
                        //ASSERT(false); // Error that should be fixed ASAP. //Removed due to crashing on linux I'll add it back once we sort out the missing list of scripts in the database.
                    }
                }
                else
                {
...
void DoOrSimulateScriptTextForMap(int32 iTextEntry, uint32 uiCreatureEntry, Map* pMap, WorldObject* pSource, Unit* target)
 {
	if (!pMap)
		return;
    ...
    {
        if (sSoundEntriesStore.LookupEntry(pData->uiSoundId))
            pSource->PlayDirectSoundToMap(pData->uiSoundId, false);
        else
            sLog->outError(LOG_FILTER_TSCR, "DoScriptText entry %i tried to process invalid sound id %u.", iTextEntry, pData->uiSoundId);
    }
 
	if (pSource) // If provided pointer for sayer, use direct version
		pSource->MonsterYellToMap(pSource, iTextEntry, pData->uiLanguage, target->GetGUID());
 }
