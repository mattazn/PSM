                ...
                if (sWorld->getBoolConfig(CONFIG_ARENA_LOG_EXTENDED_INFO))
                    for (Battleground::BattlegroundScoreMap::const_iterator itr = GetPlayerScoresBegin(); itr != GetPlayerScoresEnd(); ++itr)
                        ...
                        if (plTeamID == winnerArenaTeam->GetId())
                        {
                            ...
                            ratingChange = winnerMatchmakerChange;
                        }
                        else
                        {
                            changeType = 2; //lose
                            ...
                            ratingChange = loserMatchmakerChange;
                        }
                        std::ostringstream sql_query;
                        //                                                        gameid,              teamid,                     guid,                    changeType,             ratingChange,               teamRating,                  damageDone,                          deaths,                          healingDone,                         killingBlows,                      mapId,                 start,                   end
                        sql_query << "INSERT INTO arena_games VALUES ('" << resultTeamID << "', '" << player->GetGUID() << "', '" << changeType << "', '" << ratingChange  << "', '" << resultRating << "', '" << itr->second->DamageDone << "', '" << itr->second->Deaths << "', '" << itr->second->HealingDone << "','" << itr->second->KillingBlows << "', '" << m_MapId << "', '" << m_StartTime << "', '" << m_EndTime << "')";
                        CharacterDatabase.Execute(sql_query.str().c_str());

						if((itr->second->HealingDone + itr->second->DamageDone) <= 1000)
							isWintrade = true;
						}
						// Anti Win Trading
						if(isWintrade)
						{
							SetArenaMatchmakerRating(winner, winnerMatchmakerRating - winnerMatchmakerChange);
							SetArenaMatchmakerRating(GetOtherTeam(winner), loserMatchmakerRating + loserMatchmakerChange);
							SetArenaTeamRatingChangeForTeam(winner, winnerChange - winnerChange - winnerChange);
							SetArenaTeamRatingChangeForTeam(GetOtherTeam(winner), loserChange);
						}
            }
