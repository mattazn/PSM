...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    uint32 muteTime = 0, freezeTime = 0, banTimeMs = 0, stunTime = 0, punishment = 0;
    ...
    if (punishPlayer && !duplicatedMessage)
        PunishPlayerForBadWord(sender, muteTime, freezeTime, banTimeMs, stunTime, kickPlayer);

    free(message); //! Prevent memoryleaks by practically 'deleting' the source - sadly there's no proper way to do this in C++ so we have to use a C# method.
}
