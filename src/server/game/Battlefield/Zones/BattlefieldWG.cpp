...
bool BattlefieldWG::IncrementQuest(Player *player, uint32 quest, bool complete)
{
       if (!player)
           return false;
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
		// Handle self benefits first
			...
			if(victim->GetVehicle())
				switch(victim->GetVehicle()->GetVehicleInfo()->m_ID)
				{
				case 27881: // Catapult
		...
		if(killer->HasAura(SPELL_RECRUIT))
			PromotePlayer(killer);

		for (GuidSet::const_iterator itr = m_PlayersInWar[killerTeam].begin(); itr != m_PlayersInWar[killerTeam].end(); ++itr)
				if (Player* player = sObjectAccessor->FindPlayer(*itr))
					if (player->GetDistance2d(killer) < 20) // Reduce the range for NPC benefits.
					{
						if(player->HasAura(SPELL_RECRUIT))
							PromotePlayer(player);
					}
