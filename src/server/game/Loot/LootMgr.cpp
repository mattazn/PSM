...
bool LootStoreItem::Roll(bool rate) const
{
	...
	// Allow for a higher chance when dealing with quest items
	float questModifier = 1;
	if(pProto->Class == 12 && pProto->Quality <= 1)
		questModifier = 3;

    return roll_chance_f(chance*qualityModifier*questModifier);
}
...
void Loot::generateMoneyLoot(uint32 minAmount, uint32 maxAmount, Player* player)
{
	uint32 customMoneyRate = 0;
		...
		case 3524: // Azuremyst
			customMoneyRate = customMoneyRate*2;
			break;
		case 17: // Barrens
		case 148: // Darkshore
		case 33: // Stranglethorn
			customMoneyRate = customMoneyRate*3;
			break;
		default:
			customMoneyRate = customMoneyRate*1;
			break;
		}
		}

		if(player->HasEnoughMoney(2500000))
			customMoneyRate = 1;

    if (maxAmount > 0)
    {
        if (maxAmount <= minAmount)
            gold = uint32(maxAmount * sWorld->getRate(RATE_DROP_MONEY) * customMoneyRate);
        else if ((maxAmount - minAmount) < 32700)
            gold = uint32(urand(minAmount, maxAmount) * sWorld->getRate(RATE_DROP_MONEY) * customMoneyRate);
        else
            gold = uint32(urand(minAmount >> 8, maxAmount >> 8) * sWorld->getRate(RATE_DROP_MONEY) * customMoneyRate) << 8;
    }
}
...
void LoadLootTemplates_Fishing()
{
    ...
    // by default fishing at fail provide junk loot, entry 0 use for store this loot
    lootIdSet.erase(0);
