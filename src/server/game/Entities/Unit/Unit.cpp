...
{
    // logged overdamage dealed by players and pets
    if (damage >= 40000)
    {
		bool allow_log = true;
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            if (Player * plr = ((Player*)this))
            {
                // exclude GMs and admins
                if (plr->GetSession()->GetSecurity() >= SEC_GAMEMASTER)
                    allow_log = false;
				// Exclude people with Might of Mograine
				if(plr->HasAura(53642,0,0,0))
					allow_log = false;
				if(plr->HasItemCount(28132)) // Area 52 Special
					allow_log = false;
				if(plr->HasItemCount(34475)) // Arcane Charges
					allow_log = false;
            }
        }
        ...
        if (allow_log)
        {
            std::string att = (GetTypeId() == TYPEID_PLAYER) ? "Player" : "Pet";
            std::string def = (victim->GetTypeId() == TYPEID_PLAYER) ? "player" : "unit";
            std::string sp = spellProto ? spellProto->SpellName[sWorld->GetDefaultDbcLocale()] : "none";
			
			sLog->outWarn(LOG_FILTER_WARDEN, "ALERT: %s %s dealt higher than usual damage. (Spell: %u, Name: %s, Damage: %u) by %s with name %s", att.c_str(), GetName().c_str(), spellProto ? spellProto->Id : 0, sp.c_str(), damage, def.c_str(), victim->GetName().c_str());
			sWorld->SendGMText(12074, GetName().c_str());
		}
    }
	...
	{
		float nerfValue;
		if(ToPlayer()->GetAverageItemLevel() >= 200)
			nerfValue = 4;
		if(ToPlayer()->GetAverageItemLevel() >= 213)
			nerfValue = 8;
		if(ToPlayer()->GetAverageItemLevel() >= 215)
			nerfValue = 10;

		switch(this->GetMapId())
		{
			case 533:
				damage = CalculatePct(damage, 100-nerfValue);
				break;
			default:
				break;
		}
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (!victim->HasInArc(M_PI, this) || victim->HasUnitState(0x00000008))
    {
    ...
    if (sSpellMgr->GetSpellInfo(trigger_spell_id) == NULL)
    {
        switch (auraSpellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DEATHKNIGHT:
            {
				...
				/* Rune Strike - Not until SendFakeAuraUpdate() gets implemented.
				else if (auraSpellInfo->Id == 56816)
				{
                if( Aura * pAura = this->GetAura(56816))
    ...
    for (Unit::AuraEffectList::const_iterator itr = magnetAuras.begin(); itr != magnetAuras.end(); ++itr)
    {
        if (Unit* magnet = (*itr)->GetBase()->GetCaster())
                ...
                && _IsValidAttackTarget(magnet, spellInfo))
            {
				...
				if(magnet->isTotem())
					magnet->DealDamage(magnet, magnet->GetHealth(), 0, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, spellInfo, true);
                return magnet;
            }
    }
    return victim;
...
int32 Unit::HealBySpell(Unit* victim, SpellInfo const* spellInfo, uint32 addHealth, bool critical)
{
	if (GetTypeId() == TYPEID_PLAYER)
	{
		...
		{
			case 533:
				addHealth = CalculatePct(addHealth, 100-nerfValue);
				break;
			default:
				break;
		}
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    if (((Creature*)this)->isPet()) {
        ...
        if (pOwner && pOwner->isAlive() && !pOwner->IsFriendlyTo(enemy)) {
            if (pOwner->isInCombat()) {
                if (pOwner->GetTypeId() != TYPEID_PLAYER)
                    pOwner->SetInCombatWith(enemy);
            }
            else pOwner->SetInCombatState(PvP, enemy);
        }
     }
}
...
void CharmInfo::InitPossessCreateSpells()
{
	...
	{
			static const char *shadowpriestSpellList[] =
			{
					"48156", // Mind flay
					"48125", // SW:P
					"48158", // SW:D
			...
			static const char *holypriestSpellList[6] =
			{
					"48068", // Renew
					"48078", // Holy Nova
					"48089", // CoH
            ...
            while( strcmp(shadowpriestSpellList[i], "0") )
			{
                uint32 spell = atol((char*)shadowpriestSpellList[i++]);
...
Unit* Unit::SelectNearbyFriendly(Unit* exclude, float dist) const
{
    std::list<Unit*> targets;
...
Unit* Unit::SelectNearbyFriendlyMissingBuff(Unit* exclude, float dist, uint32 spell) const
{
    std::list<Unit*> targets;
    Trinity::FriendlyMissingBuffInRange u_check(this, dist, spell);
    Trinity::UnitListSearcher<Trinity::FriendlyMissingBuffInRange> searcher(this, targets, u_check);
    VisitNearbyObject(dist, searcher);
...
Unit* Unit::SelectNearbyFriendlyCCed(Unit* exclude, float dist) const
{
    std::list<Unit*> targets;
    Trinity::FriendlyCCedInRange u_check(this, dist);
    Trinity::UnitListSearcher<Trinity::FriendlyCCedInRange> searcher(this, targets, u_check);
    VisitNearbyObject(dist, searcher);
...
Unit* Unit::SelectNearbyFriendlyMissingMostHP(Unit* exclude, float dist, uint32 hp) const
{
    std::list<Unit*> targets;
    Trinity::MostHPMissingInRange u_check(this, dist, hp);
    Trinity::UnitListSearcher<Trinity::MostHPMissingInRange> searcher(this, targets, u_check);
    VisitNearbyObject(dist, searcher);
...
{
	// Prevent honor farming someone on your IP
	if (GetTypeId() == TYPEID_PLAYER && victim->GetTypeId() == TYPEID_PLAYER)
	{
		if(GetName() != victim->GetName())
			if (ToPlayer()->GetSession()->GetRemoteAddress() == victim->ToPlayer()->GetSession()->GetRemoteAddress())
			{
				...
				sLog->outWarn(LOG_FILTER_WARDEN, "Player '%s' (IP %s) killed player '%s' who uses the same IP address. Possible honor farm, both players have been kicked.", ToPlayer()->GetName().c_str(), ToPlayer()->GetSession()->GetRemoteAddress(), victim->ToPlayer()->GetName().c_str());
