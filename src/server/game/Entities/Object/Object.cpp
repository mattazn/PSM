...
void WorldObject::MonsterYellToMap(WorldObject* guid, int32 textId, uint32 language, uint64 TargetGuid)
{
    Trinity::MonsterChatBuilder say_build(*this, CHAT_MSG_MONSTER_YELL, textId, language, TargetGuid);
    Trinity::LocalizedPacketDo<Trinity::MonsterChatBuilder> say_do(say_build);
...
void WorldObject::PlayDirectSoundToMap(uint32 sound_id, uint32 zoneId /*=0*/)
{
    WorldPacket data(SMSG_PLAY_SOUND, 4);
    data << uint32(sound_id);
    data << uint64(GetGUID());

     Map::PlayerList const &PlayerList = this->GetMap()->GetPlayers();
