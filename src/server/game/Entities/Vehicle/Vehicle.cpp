...
void Vehicle::ApplyAllImmunities()
{
    ...
    if (_me->ToCreature() && _me->ToCreature()->GetCreatureTemplate()->type == CREATURE_TYPE_MECHANICAL && !_me->ToCreature()->isWorldBoss())
    {
		...
		// Specific spell immunities for vehicles 
		_me->ApplySpellImmune(0, IMMUNITY_ID, 53601, true); // Sacred Shield
		_me->ApplySpellImmune(0, IMMUNITY_ID, 17, true);    // Power Word: Shield
	}
	...
	switch (_me->GetEntry())
	{
		...
		case 30236: // Icecrown		- Argent Cannon
		case 28366: // Wintergrasp  - Tower Cannon
			_me->SetControlled(true, UNIT_STATE_ROOT);
			_me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_DECREASE_SPEED, true);
		break;
		case 34812: // Pilgrims Bounty Chairlist begin
		case 34819:
		case 34822:
		case 34823:
		case 34824: // Pilgrims bounty Chairlist end
			_me->SetControlled(true, UNIT_STATE_ROOT);
		break;
	}
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    if (seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_HAS_VEH_EXIT_ANIM_FORCED_EXIT || seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
    {
        switch (_me->GetEntry())
        {
			...
			case 28817: // Death Knight Mine Cart
            default:
                unit->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                break;
        }
        unit->RemoveAurasByType(SPELL_AURA_MOD_SHAPESHIFT);
    }
