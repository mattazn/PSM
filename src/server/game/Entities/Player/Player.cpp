...
inline void KillRewarder::_RewardXP(Player* player, float rate)
{
    ...
    if (_group)
    {
        // Gather the lowest XP rate of the party member and use that.
        for (GroupReference* itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            if (Player* member = itr->getSource())
            {
				if(member->GetXpRate() < member->getGroupXP())
					member->setGoupXP(member->GetXpRate());
            }
        }
...
void Player::GiveLevel(uint8 level)
{
	...
	case 58:
		if(GetXpRate() >= 16)
		{
		GetSession()->SendNotification("Congratulations on reaching level 58. Your XP rate has been reduced to x15.");
		SetXpRate(15);
		}
	break;
	case 68:
		if(GetXpRate() >= 6)
		{
		GetSession()->SendNotification("Congratulations on reaching level 68. Your XP rate has been reduced to x5.");
		SetXpRate(5);
		}
	break;
	case 80:
...
void Player::UpdateCombatSkills(Unit* victim, WeaponAttackType attType, bool defence)
{
	...
	if(GetXpRate() >= 0)
		chance = chance < 1.0f ? 1.0f : chance;                 //minimum chance to increase skill is 25% 1/4
	if(GetXpRate() >= 6)
		chance = chance < 15.0f ? 15.0f : chance;                 //minimum chance to increase skill is 15% 1/6
	if(GetXpRate() >= 16)
		chance = chance < 25.0f ? 25.0f : chance;                 //minimum chance to increase skill is 1%
...
void Player::SendLoot(uint64 guid, LootType loot_type)
{
    ...
    if (IS_GAMEOBJECT_GUID(guid))
    {
        ...
        if (!go || (loot_type != LOOT_FISHINGHOLE && (loot_type != LOOT_FISHING && loot_type != LOOT_FISHING_FAIL || go->GetOwnerGUID() != GetGUID()) && !go->IsWithinDistInMap(this, INTERACTION_DISTANCE)) || (loot_type == LOOT_CORPSE && go->GetRespawnTime() && go->isSpawnedByDefault()))
        {
        ...
        if (go->getLootState() == GO_READY)
        {
            ...
            // Entry 0 in fishing loot template used for store junk fish loot at fishing fail
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    // 67		68
    //"xp_rate, achiDisabled FROM characters WHERE guid = '%u'", guid);
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
	...
	m_xp_rate = fields[67].GetUInt8();

		achiDisabled = fields[68].GetBool();
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    if (create)
    {
		...
		stmt->setUInt8(index++, m_xp_rate);
		stmt->setBool(index++, achiDisabled);
    }
    else
    {
        // Update query
