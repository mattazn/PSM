...
bool PlayerMovementChecker::canMove()
{
	...
	if(_player->isDead())
		return true;
    if(_player->HasAura(51852))
        return true;
    if(_player->IsPolymorphed() && _player->GetMap()->IsBattlegroundOrArena())
        return true;
	if(_player->GetMapId() == 520 && _player->isInCombat()) // Nexus Omorok knock up
		return true;
	if(_player->GetMapId() == 578) // Oculus
		return true;
	if(_player->GetMapId() == 535 && _player->isInCombat()) // Naxxramas
		return true;
	if(_player->GetMapId() == 527) // EoE
		return true;
	if(_player->isInFlight())
		return true;
	if(_player->HasItemCount(28132)) // Area 52 Special
		return true;
	if(_player->HasItemCount(39700)) // Horn of the Frostbrood
		return true;
	if(_player->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT))
		return true;
...
bool PlayerMovementChecker::isFlyingOk()
{
    ...
    const bool justHadFlyer = flyTimeDelta < _latency * 2 + _timediff * 2 + 10000;

    const bool passed = canFly || (_player->HasUnitState(UNIT_STATE_ROAMING|UNIT_STATE_ROAMING_MOVE|UNIT_STATE_CHARGING) || _justCharged || justHadFlyer);

    if(canFly)
        _lastFlyingTime = _currServerTime;

    if(!passed)
        sLog->outWarn(LOG_FILTER_WARDEN, "MovementChecker: flying. Player: %s [GUID: %u]. canFly: %s, flyDelta: %u",
            _player->GetName().c_str(), _player->GetGUIDLow(), canFly ? "yes" : "no", flyTimeDelta);
...
bool PlayerMovementChecker::isPlaneOk()
{
    ...
    if(_player->HasUnitState(UNIT_STATE_ROAMING|UNIT_STATE_ROAMING_MOVE|UNIT_STATE_CHARGING))
        return true;
    ...
    {
        sLog->outWarn(LOG_FILTER_WARDEN, "MovementChecker: plane. Player: %s [GUID: %u].", _player->GetName().c_str(), _player->GetGUIDLow());
        return false;
    }
...
bool PlayerMovementChecker::isJumpOk()
{
    ...
    if(_alreadyJumped)
    {
        sLog->outWarn(LOG_FILTER_WARDEN, "MovementChecker: jump. Player: %s [GUID: %u].", _player->GetName().c_str(), _player->GetGUIDLow());
        return false;
    }
...
bool PlayerMovementChecker::isFeatherFallOk()
{
    ...
    {
        sLog->outWarn(LOG_FILTER_WARDEN, "MovementChecker: FeatherFall. Player: %s [GUID: %u].", _player->GetName().c_str(), _player->GetGUIDLow());
        return false;
    }
...
bool PlayerMovementChecker::isWaterWalkOk()
{
    ...
    if(!hasWaterwalkAura && !justHadWaterwalkAura && !(_player->HasUnitState(UNIT_STATE_ROAMING|UNIT_STATE_ROAMING_MOVE|UNIT_STATE_CHARGING) || _justCharged))
    {
        sLog->outWarn(LOG_FILTER_WARDEN, "MovementChecker: waterwalking. Player: %s [GUID: %u].", _player->GetName().c_str(), _player->GetGUIDLow());
        return false;
    }
...
bool PlayerMovementChecker::isSpeedOk()
{
    ...
    {
        sLog->outWarn(LOG_FILTER_WARDEN, "MovementChecker: speed. Player: %s [GUID: %u]. Allowed dist: %f, client dist: %f, 3D dist: %f, clientDelta: %u, serverDelta: %u, moveType: %u, speed: %f",
            _player->GetName().c_str(), _player->GetGUIDLow(), allowedDistance, _distance2D, _distance3D, clientTimeDelta, serverTimeDelta, moveType, speedToCheck);
        return false;
    }
...
bool PlayerMovementChecker::isZCoordOk()
{
    ...
    {
        sLog->outWarn(LOG_FILTER_WARDEN, "MovementChecker: ZCoord. Player: %s [GUID: %u], deltaZ: %f, lastZ: %f, floorZ: %f, height: %f",
            _player->GetName().c_str(), _player->GetGUIDLow(), deltaZ, _lastZCoord, _floorZ, _moverHeight);
        return false;
    }
...
void PlayerMovementChecker::doMovementCorrection()
{
	...
	sWorld->SendGMText(12073, _player->GetName().c_str());
}
