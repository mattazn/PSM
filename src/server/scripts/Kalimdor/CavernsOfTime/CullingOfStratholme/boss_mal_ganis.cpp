...
public:
    ...
    struct boss_mal_ganisAI : public ScriptedAI
    {
        boss_mal_ganisAI(Creature* creature) : ScriptedAI(creature)
        {
			...
			me->ApplySpellImmune(2, IMMUNITY_MECHANIC, MECHANIC_FEAR, true);
            ...
            switch (Phase)
            {
                case COMBAT:
                    ...
                    if (HealthBelowPct(1))
                    {
						...
						me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_UNK_6 | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_UNK_15 | UNIT_FLAG_NOT_SELECTABLE);
                        uiOutroStep = 1;
						if (Creature* pArthas = me->GetCreature(*me, instance ? instance->GetData64(DATA_ARTHAS) : 0))
							pArthas->CombatStop(true);
                        Phase = OUTRO;
                        return;
                    }
