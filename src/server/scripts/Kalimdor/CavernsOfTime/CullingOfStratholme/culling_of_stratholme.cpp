...
#define GOSSIP_ITEM_ARTHAS_0 "I'm ready to start the Culling of Stratholme."
#define GOSSIP_ITEM_ARTHAS_1 "Yes, my Prince. We're ready."
#define GOSSIP_ITEM_ARTHAS_2 "We're only doing what is best for Lordaeron your Highness."
...
#define GOSSIP_ITEM_ARTHAS_6 "[Debug Option] Begin at Mal'ganis."
...
public:
    ...
    struct npc_arthasAI : public npc_escortAI
    {
        ...
        uint64 bossGUID; //uiMeathookGUID || uiSalrammGUID
        uint64 epochGUID;
        uint64 malganisGUID;
        uint64 infiniteGUID;
                ...
                if (phaseTimer <= diff)
                {
                    switch (step)
                    {
                        ...
                        case 1:
                            ...
                            {
								/*if(Unit* pKnightOne = uther->SummonCreature(NPC_SILVER_KNIGHT, 1828.93f, 1279.01f, 143.28f, 1.37f, TEMPSUMMON_TIMED_DESPAWN, 70000))
									pKnightOne->GetMotionMaster()->MovePoint(0, 1883.79f, 1284.59f, 143.80f);
								if(Unit* pKnightTwo = uther->SummonCreature(NPC_SILVER_KNIGHT, 1828.93f, 1279.01f, 143.28f, 1.37f, TEMPSUMMON_TIMED_DESPAWN, 70000))
									pKnightTwo->GetMotionMaster()->MovePoint(0, 1883.80f, 1287.21f, 143.86f);
								if(Unit* pKnightThree = uther->SummonCreature(NPC_SILVER_KNIGHT, 1828.93f, 1279.01f, 143.28f, 1.37f, TEMPSUMMON_TIMED_DESPAWN, 70000))
									pKnightThree->GetMotionMaster()->MovePoint(0, 1883.91f, 1281.72f, 143.76f);*/
                        ...
                        case 36:
                            if (Creature* malganis = me->SummonCreature(NPC_MAL_GANIS, 2117.349f, 1288.624f, 136.271f, 1.37f, TEMPSUMMON_TIMED_DESPAWN, 60000))
                            {
                                if (Creature* pStalkerM = Unit::GetCreature(*me, stalkerGUID))
                                    malganis->CastSpell(pStalkerM, 51908, false);
                        ...
                        case 40:
                            ...
                            if(IsHeroic() && instance->GetData(DATA_INFINITE_EVENT) != IN_PROGRESS && instance->GetData(DATA_INFINITE_EVENT) != FAIL)
                        ...
                        case 57:
                            if (instance->GetData(bossEvent) != DONE)
                            {
                                ...
                                if (instance->GetData(DATA_MEATHOOK) == DONE)
									instance->DoUpdateWorldState(WORLDSTATE_WAVE_COUNT, wave+1);
								else
									instance->DoUpdateWorldState(WORLDSTATE_WAVE_COUNT, wave);
                            }
                            JumpToNextStep(500);
                            break;
                        ...
                        case 59:
                            if (instance->GetData(bossEvent) != DONE)
                            {
                                ...
                                if (Unit* pBoss = me->SummonCreature(uiBossID, x, y, z, o, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 900000))
                                {
                                    bossGUID = pBoss->GetGUID();
                                    pBoss->SetWalk(true);
                                    pBoss->GetMotionMaster()->MovePoint(0, x2, y2, z2);
                                }
                            }
                        ...
                        case 68:
                            if (Creature* disguised2 = Unit::GetCreature(*me, infiniteDraconianGUID[2]))
                            {
								disguised2->CastSpell(me, SPELL_TRANSFORM);
                        ...
                        case 69:
                            if (Creature* disguised1 = Unit::GetCreature(*me, infiniteDraconianGUID[1]))
                            {
								disguised1->CastSpell(me, SPELL_TRANSFORM);
                        ...
                        case 70:
                            if (Creature* disguised0 = Unit::GetCreature(*me, infiniteDraconianGUID[0]))
                            {
								disguised0->CastSpell(me, SPELL_TRANSFORM);
        ...
        {
            if(Creature* creature = me->GetCreature(*me, instance->GetData64(DATA_CRIER)))
            {
                int32 textId = 0;
                switch(waveNumber)
                {
                    case 0: case 7: textId = -1595048; break; // King Square
                    case 1: case 4: textId = -1595059; break; // Festival Street Fountain
                    case 2: case 5: textId = -1595050; break; // Council
                    case 3: case 6: textId = -1595051; break; // King Square Fountain
                }
                if(textId < 0)
                    creature->YellToZone(textId, LANG_UNIVERSAL, 0);
            }
        }
    };
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if(pInstance)
        {
            ...
            if(pInstance->GetData(DATA_CRATE_COUNT) < 5)
            {
				...
				//player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_CHROMIE_2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+2);
...
class npc_cos_zombie : public CreatureScript
{
    public:
        ...
        {
            npc_cos_zombieAI(Creature* pCreature): ScriptedAI(pCreature)
            {
        ...
        {
            return new npc_cos_zombieAI(pCreature);
        }
};
