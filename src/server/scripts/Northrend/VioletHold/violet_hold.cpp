...
#define PREFIGHT_SPAWN_TIME 8000
...
enum Spells
{
	...
	SPELL_ARCANE_SPHERE_AURA		  = 44263
};
...
enum PortalEmotes
{
	PORTAL_GUARDIAN										  = -1608056,
	PORTAL_SQUAD										  = -1608057,
	PORTAL_KEEPER										  = -1608058
};
enum eSinclari
{
    SAY_SINCLARI_1                    = -1608045,
    SAY_SINCLARI_2                    = -1608046
};
            ...
            if (uiPhase)
            {
                if (uiTimer <= uiDiff)
                {
                    switch (uiPhase)
                    {
                        case 1:
							{
							 Creature* pPreEventPortal1 = Unit::GetCreature((*me),instance->GetData64(DATA_PRE_EVENT_PORTAL_1));
							 if (pPreEventPortal1)
							 pPreEventPortal1->DisappearAndDie();
							 Creature* pPreEventPortal2 = Unit::GetCreature((*me),instance->GetData64(DATA_PRE_EVENT_PORTAL_2));
							 if (pPreEventPortal2)
							 pPreEventPortal2->DisappearAndDie();
							Creature* defenseSystem = Unit::GetCreature((*me),instance->GetData64(DATA_DEFENSE_SYSTEM));
							if(defenseSystem)
								defenseSystem->CastSpell(defenseSystem, SPELL_ARCANE_SPHERE_AURA);
                        ...
                        case 4:
                            ...
                            break;
						case 5:
                             me->SetFacingTo(0.034811f);
                             DoScriptText(SAY_SINCLARI_2, me);
                             uiTimer = 2000;
                             uiPhase = 6;
                             break;
                        case 6:
...
public:
    ...
    struct mob_azure_saboteurAI : public npc_escortAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
			...
			if(uiDespawnTime <= diff)
			{
				me->DisappearAndDie();
			} else uiDespawnTime -= diff;
        }
    ...
    struct npc_teleportation_portalAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if ((uiWaveCount == 6) || (uiWaveCount == 12)) //Don't spawn mobs on boss encounters
				...
				if (uiWaveCount == 0 && (GetData(DATA_MAIN_DOOR) != GO_STATE_READY))
				{
				if (uiPreFightSpawnTimer <= diff)
				{
				uint8 j=1;
				for (uint8 i = 0; i < j; ++i)
				{
				DoSummon(CREATURE_AZURE_INVADER_1, me, 2.0f, 20000, TEMPSUMMON_DEAD_DESPAWN);
				}
				uiPreFightSpawnTimer = 17000;
				}
				else uiPreFightSpawnTimer -= diff;
				return;
				}
                ...
                case 1:
                    if (uiSpawnTimer <= diff)
                    {
                        ...
                        else
                        {
							...
							if(entry == CREATURE_PORTAL_GUARDIAN)
								DoScriptText(PORTAL_GUARDIAN,me);
							if(entry == CREATURE_PORTAL_KEEPER)
								DoScriptText(PORTAL_KEEPER,me);
                            if (Creature* pPortalKeeper = DoSummon(entry, me, 2.0f, 0, TEMPSUMMON_DEAD_DESPAWN))
                                me->CastSpell(pPortalKeeper, SPELL_PORTAL_CHANNEL);
        ...
        void JustSummoned(Creature* summoned)
        {
            ...
            summoned->CastSpell(summoned, SPELL_TELEPORT);
        }
...
struct violet_hold_trashAI : public npc_escortAI
{
    ...
    public:
		...
		if(me->isType(2) || me->isType(7))
		{
			uint8 textSelection = urand(1,8);

			switch(textSelection)
			{
			case 1:
			DoScriptText(TRASH_SAY_1, me);
			break;
			case 2:
			DoScriptText(TRASH_SAY_2, me);
			break;
			case 3:
			DoScriptText(TRASH_SAY_3, me);
			break;
			case 4:
			DoScriptText(TRASH_SAY_4, me);
			break;
			case 5:
			DoScriptText(TRASH_SAY_5, me);
			break;
			case 6:
			DoScriptText(TRASH_SAY_6, me);
			break;
			case 7:
			DoScriptText(TRASH_SAY_7, me);
			break;
			case 8:
			DoScriptText(TRASH_SAY_8, me);
			break;
			}
		}
		}
    ...
    void CreatureStartAttackDoor()
    {
        me->SetReactState(REACT_DEFENSIVE);
                //if (Creature* prisondoor = Unit::GetCreature((*me), instance->GetData64(DATA_PRISON_DOOR)))
                //{
...
public:
    ...
    struct mob_azure_raiderAI : public violet_hold_trashAI
    {
        ...
        void Reset()
        {
            uiConcussionBlowTimer = 1500+rand()%3500;
            uiMagicReflectionTimer = 4000+rand()%4000;
        }
...
public:
    ...
    struct npc_defense_systemAI : public ScriptedAI
    {
        npc_defense_systemAI(Creature* creature) : ScriptedAI(creature)
        {
            instance = creature->GetInstanceScript();
        }
        ...
        void UpdateAI(const uint32 diff)
        {
			if((uiZapTimer <= diff) && (!me->HasAura(SPELL_ARCANE_SPHERE_AURA)))
			{
				uiZapTimer = 2000;
			}
			...
			if((uiZapTimer <= diff) && (me->HasAura(SPELL_ARCANE_SPHERE_AURA)))
			{
				...
				uiFadeTimer = 2000;
			} else uiZapTimer -= diff;

			if((uiFadeTimer <= diff) && (me->HasAura(SPELL_ARCANE_SPHERE_AURA)))
			{
				uiFadeTimer = 0;
				me->RemoveAura(SPELL_ARCANE_SPHERE_AURA);
			} else uiFadeTimer -= diff;
        }
    };
};

class VioletHold_ActivationCrystals : public GameObjectScript
{
    public:
        VioletHold_ActivationCrystals() : GameObjectScript("VioletHold_ActivationCrystals") { }
		
        bool OnGossipHello(Player* player, GameObject* go)
        {
			if(InstanceScript* instance = go->GetInstanceScript()){
			Creature* defenseSystem = Unit::GetCreature((*go),instance->GetData64(DATA_DEFENSE_SYSTEM));
			if(defenseSystem)
				defenseSystem->CastSpell(defenseSystem, SPELL_ARCANE_SPHERE_AURA);
			}
...
void AddSC_violet_hold()
{
	...
	new VioletHold_ActivationCrystals();
}