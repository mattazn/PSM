...
const Position BossStartMove2  = {1877.883667f, 846.944153f, 43.333626f, 0.0f};
const Position BossStartMove21 = {1881.423340f, 863.388977f, 43.333626f, 0.0f};
const Position BossStartMove22 = {1881.423340f, 851.553406f, 43.333626f, 0.0f};
...
const Position PreEventPortal1Location = {1877.51f, 850.104f, 44.6599f, 4.7822f};
const Position PreEventPortal2Location = {1890.64f, 753.471f, 48.7224f, 1.71042f};
...
public:
    ...
    struct instance_violet_hold_InstanceMapScript : public InstanceScript
    {
        ...
        uint64 uiSinclari;
        uint64 uiPrisonDoor;
		uint64 uiPreEventPortal1;
		uint64 uiPreEventPortal2;
        ...
        void OnCreatureCreate(Creature* creature)
        {
            ...
            {
                case DATA_DEFENSE_SYSTEM:
                    uiDefenseSystem = creature->GetGUID();
                    break;
                case DATA_PRISON_DOOR:
                    uiPrisonDoor = creature->GetGUID();
                    break;
        ...
        uint32 GetData(uint32 type) const
        {
            switch (type)
            {
                case DATA_PRISON_DOOR:              return uiPrisonDoor;
        ...
        uint64 GetData64(uint32 identifier) const
        {
            switch (identifier)
            {
				...
				case DATA_PRE_EVENT_PORTAL_1:		return uiPreEventPortal1;
				case DATA_PRE_EVENT_PORTAL_2:		return uiPreEventPortal2;
				case DATA_DEFENSE_SYSTEM:			return uiDefenseSystem;
            }
        ...
        void Update(uint32 diff)
        {
            ...
            if (bActive && uiMainEventPhase == IN_PROGRESS)
            {
                if (uiActivationTimer < diff)
                {
                    ...
                    uiActivationTimer = (uiWaveCount == 6 || uiWaveCount == 12) ? 45000 : 4000;
            ...
            if ( uiMainEventPhase == IN_PROGRESS && CheckWipe())
            {
				...
				SetData(DATA_DOOR_INTEGRITY, 100);
                uiMainEventPhase = NOT_STARTED;
        ...
        void ActivateCrystal()
        {
            ...
            for (std::set<uint64>::const_iterator itr = trashMobs.begin(); itr != trashMobs.end(); ++itr)
            {
                ...
                if (creature && creature->isAlive())
                    creature->DealDamage(creature, 50000);
            }
        }
