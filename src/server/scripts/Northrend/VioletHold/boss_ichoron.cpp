...
public:
    ...
    struct boss_ichoronAI : public ScriptedAI
    {
        ...
        void DoAction(const int32 param)
        {
            ...
            switch (param)
            {
                ...
                case ACTION_WATER_ELEMENT_KILLED:
                    uint32 damage = me->CountPctFromMaxHealth(2);
        ...
        {
            if (!UpdateVictim() && !me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE|UNIT_FLAG_DISABLE_MOVE))
                return;
                ...
                if (uiBubbleCheckerTimer <= uiDiff)
                {
                    if (!bIsExploded)
                    {
                        if (!me->HasAura(SPELL_PROTECTIVE_BUBBLE, 0))
                        {
							...
							me->DealDamage(me, me->CountPctFromMaxHealth(25));
