    ...
    struct mob_ethereal_sphereAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (uiSummonPlayers_Timer < uiDiff)
            {
                if (instance)
                {
                    if (Creature* pXevozz = Unit::GetCreature(*me, instance->GetData64(DATA_XEVOZZ)))
                    {
						...
						if(me->IsWithinLOS(x,y,z))
						{
							...
							{
								pXevozz->NearTeleportTo(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), pXevozz->GetOrientation(), false);
                ...
                uiSummonPlayers_Timer = urand(15000, 25000);
            }
