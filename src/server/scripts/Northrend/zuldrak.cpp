...
{
    QUEST_IT_ROLLS_DOWNHILL         = 12673,
    GAMEOBJECT_CRYSTALLIZED_BLIGHT  = 190940,
    NPC_BLIGHT_CRYSTAL_KC           = 28740,
    SPELL_CHARM_GEIST               = 52244,
    ...
    public:
        ...
        struct npc_blight_geistAI : public ScriptedAI
        {
			...
			void SpellHit(Unit* /*caster*/, SpellInfo const* spell)
			{
				...
				{
					if (Creature* crystallizedBlight = me->FindNearestCreature(NPC_BLIGHT_CRYSTAL_KC, 10.0f, true))
                       me->DespawnOrUnsummon();
						//if (me->GetOwner() && me->GetOwner()->ToPlayer())
                           //me->GetOwner()->ToPlayer()->KilledMonsterCredit(NPC_BLIGHT_CRYSTAL_KC, 0);					
				}
			}
            /*void UpdateAI(uint32 const diff)
            {
                //if (stepTimer <= diff)
                //{
                    //if (me->HasAura(SPELL_CHARM_GEIST)/* && !executingEvent)
                    //{
                        //if (me->GetOwner() && me->GetOwner()->ToPlayer() && me->GetOwner()->ToPlayer()->GetQuestStatus(QUEST_IT_ROLLS_DOWNHILL) == QUEST_STATUS_INCOMPLETE)
                        //{
                            //if (GameObject* crystallizedBlight = me->FindNearestGameObject(GAMEOBJECT_CRYSTALLIZED_BLIGHT, 10.0f))
                            //{
							if (Creature* crystallizedBlight = me->FindNearestCreature(NPC_BLIGHT_CRYSTAL_KC, 10.0f, true))
							{
                                executingEvent = true;
                                ...
                                switch (step)
                                {
                                    case 0:
                                        ...
                                        break;
                                    case 1:
                                        me->GetMotionMaster()->MovePoint(0, crystallizedBlight->GetPositionX()+1, crystallizedBlight->GetPositionY(), crystallizedBlight->GetPositionZ());
                                        break;
                                    case 2:
                                        if (me->GetOwner() && me->GetOwner()->ToPlayer())
                                            me->GetOwner()->ToPlayer()->KilledMonsterCredit(NPC_BLIGHT_CRYSTAL_KC, 0);

                                        crystallizedBlight->RemoveFromWorld();
                                        me->DespawnOrUnsummon();
                                        break;
									default:
										me->Kill(me, false);
...
enum BringingDownHebjin
{
    QUEST_BRINGING_DOWN_HEBJIN  = 12662,

    ENTRY_HEBJIN                = 28636,
    ENTRY_HEBJINS_BAT           = 28639,
};

class go_hebjins_drums : public GameObjectScript
{
    public:
        go_hebjins_drums() : GameObjectScript("go_hebjins_drums") { }

        bool OnGossipHello(Player* player, GameObject* go)
        {
            if (player->GetQuestStatus(QUEST_BRINGING_DOWN_HEBJIN) == QUEST_STATUS_INCOMPLETE)
            {
                if (!go->FindNearestCreature(ENTRY_HEBJIN, 50.0f, true) && !go->FindNearestCreature(ENTRY_HEBJINS_BAT, 50.0f, true))
                {
                    if (Creature* hebjin = player->SummonCreature(ENTRY_HEBJIN, 5928.750488f, -3815.419189f, 364.704163f, 2.450581f, TEMPSUMMON_TIMED_DESPAWN, 300000))
                    {
                        if (Creature* bat = player->SummonCreature(ENTRY_HEBJINS_BAT, 5928.750488f, -3815.419189f, 364.704163f, 2.450581f, TEMPSUMMON_TIMED_DESPAWN, 300000))
                        {
                            hebjin->AI()->AttackStart(player);
                            bat->AI()->AttackStart(player);
                        }
                    }
                }
            }
