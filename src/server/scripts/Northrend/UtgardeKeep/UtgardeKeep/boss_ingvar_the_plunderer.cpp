...
enum eSpells
{
	YELL_RESSURECT_1 = -1999936,
	YELL_RESSURECT_2 = -1999937,
...
public:
    ...
    struct mob_annhylde_the_callerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (ingvar)
            {
				ingvar->GetPosition(&IngvarsCorpse);
				
				me->NearTeleportTo(IngvarsCorpse.GetPositionX(), IngvarsCorpse.GetPositionY()-4, IngvarsCorpse.GetPositionZ()+35,IngvarsCorpse.GetOrientation());
				me->GetMotionMaster()->MovePoint(1, IngvarsCorpse.GetPositionX(), IngvarsCorpse.GetPositionY()-4, IngvarsCorpse.GetPositionZ()+15);
				...
				{
				case 0:
				DoScriptText(YELL_RESSURECT_1, me);
				break;
				case 1:
				DoScriptText(YELL_RESSURECT_2, me);
				break;
				}
            }
        }
                ...
                if (uiResurectTimer <= diff)
                {
                    ...
                    else if (uiResurectPhase == 2)
                    {
                        if (Creature* ingvar = Unit::GetCreature(*me, instance ? instance->GetData64(DATA_INGVAR) : 0))
                        {
                            ...
                            me->GetMotionMaster()->MovePoint(2, x, y+8, z+30);
                            ++uiResurectPhase;
                            uiResurectTimer = 0;
                        }
                    }
