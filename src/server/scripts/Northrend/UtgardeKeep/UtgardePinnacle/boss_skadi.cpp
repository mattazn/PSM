...
enum eYells
{
    ...
    SAY_KILL_1                          = -1575005,
    SAY_KILL_2                          = -1575006,
    EMOTE_BREATH                        = -1575007, //Grauf
    SAY_DEATH                           = -1575008,
    EMOTE_RANGE                         = -1575009, //Skadi
...
static Position Location[]=
{
    ...
    {400.402802f, -543.112244f, 150.847397f, 0},         // 2
    {521.071167f, -549.356995f, 137.554504f, 0}, // 3
    ...
    // Breach Zone
    {484.559296f, -514.648804f, 123.863182f, 0},      //69
    {463.483673f, -514.214172f, 116.741493f, 0},      //70
    {367.488495f, -510.867706f, 115.568871f, 0},      //71
	{338.311951f, -510.406067f, 115.499619f, 0},      //72
	{338.311951f, -510.406067f, 115.499619f, 0},      //73
};
...
public:
    ...
    struct boss_skadiAI : public ScriptedAI
    {
        ...
        void DoAction(const int32 action)
        {
            if(action == 1)
            {
                ...
                if(m_uiSpellHitCount >= DUNGEON_MODE(MAX_HIT_COUNT, H_MAX_HIT_COUNT))
                {
					...
					DoTeleportTo(Location[4].GetPositionX(), Location[4].GetPositionY(), Location[4].GetPositionZ(), 0);
                    me->GetMotionMaster()->MoveJump(Location[4].GetPositionX(), Location[4].GetPositionY(), Location[4].GetPositionZ(), 16.0f, 21.0f);
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);
                    DoScriptText(SAY_DRAKE_DEATH, me);
                    m_uiCrushTimer = 10000;
                    m_uiPoisonedSpearTimer = 8000;
                    m_uiWhirlwindTimer = 9000;
                }
            }
        }
        ...
        {
            if(spell->Id == (uint8)DUNGEON_MODE(SPELL_POISONED_SPEAR, H_SPELL_POISONED_SPEAR))
                    ...
                    if (m_uiMovementTimer <= diff)
                    {
                        switch (m_uiWaypointId)
                        {
                            ...
                            case 6:
                                ...
                                break;
                            case 7:
                                me->GetMotionMaster()->MovePoint(0, Location[73].GetPositionX(), Location[73].GetPositionY(), Location[73].GetPositionZ());
                                m_uiMovementTimer = 2000;
                                break;
                            case 8:
                                me->GetMotionMaster()->MovePoint(0, Location[1].GetPositionX(), Location[1].GetPositionY(), Location[1].GetPositionZ());
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            //if(IsHeroic() && m_bAchiev)
                //instance->DoCompleteAchievement(ACHIEV_MY_GIRL_LOVES_TO_SKADI);
        }
