...
public:
    ...
    struct boss_sartharionAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            // Drakes respawning system & Fire Cyclones!!
            if (instance)
			{
                ...
                uint32 count = pFireCyclonesList.size();

                for(uint32 i = 0; i < count; ++i)
				{
					(*itr)->RemoveAurasDueToSpell(SPELL_LAVA_VISUAL, 0, 0, AURA_REMOVE_BY_DEFAULT);
					(*itr)->SetSpeed(MOVE_FLIGHT, 0.2f, true);
					++itr;
				}
            }
        }
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            if (instance)
            {
                ...
                for(uint32 i = 0; i < count; ++i)
				{
					(*itr)->CastSpell((*itr), SPELL_LAVA_VISUAL, true);
					(*itr)->SetSpeed(MOVE_FLIGHT, 0.2f, true);
					(*itr)->SetSpeed(MOVE_WALK, 0.2f, true);
					++itr;
				}
            }
        }
                ...
                if(m_FireCycloneFireing)
                {
                    if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
					{
						...
						if(bChance)
						{
                            ...
                            m_FireCycloneFireing = false;
                            m_guidFireCyclone = 0;
                            m_uiLavaStrikeTimer = (m_bIsSoftEnraged ? urand(1400, 2000) : urand(5000, 12500));
						}
					} 
				} else {
...
struct dummy_dragonAI : public ScriptedAI
{
    ...
    {
		if(instance->GetData(TYPE_SARTHARION_EVENT) == IN_PROGRESS)
			return me->SummonGameObject(uiId, fX, fY, fZ, fAngle, 0, 0, 0, 0, uiDespawntime);
		else
    ...
    void OpenPortal()
    {
        ...
        GameObject* pPortal;
		if(instance->GetData(TYPE_SARTHARION_EVENT) == IN_PROGRESS)
			pPortal = DoSpawnGameobject(GO_TWILIGHT_PORTAL,3248.8723f, 533.1899f, 58.9599f,me->GetOrientation(), 0);
		else
			pPortal = DoSpawnGameobject(GO_TWILIGHT_PORTAL,(float)(rand()%10),(float)(rand()%10),-1.0f,me->GetOrientation(), 0);
    ...
    void JustDied(Unit* /*killer*/)
    {
		Map* map = me->GetMap();
		Map::PlayerList const &PlayerList = map->GetPlayers();
		if(instance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS && instance->GetData(TYPE_SARTHARION_EVENT) != DONE)
			for (Map::PlayerList::const_iterator itr = PlayerList.begin(); itr != PlayerList.end(); ++itr)
				if (Player* player = itr->getSource())
					if(this->Is25ManRaid())
						player->AddItem(ITEM_EMBLEM_OF_VALOR,1);
					else
    ...
    struct mob_acolyte_of_shadronAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (instance)
            {
                ...
                else if (instance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS)
                {
        ...
        void JustDied(Unit* /*killer*/)
        {
            if (instance)
            {
                ...
                if (Shadron)
                    (CAST_AI(mob_shadron::mob_shadronAI, Shadron->AI()))->DoAction(ACTION_CLOSE_PORTAL);
    ...
    struct mob_acolyte_of_vesperonAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            if (instance)
            {
                ...
                if (pVesperon)
                    (CAST_AI(mob_vesperon::mob_vesperonAI, pVesperon->AI()))->DoAction(ACTION_CLOSE_PORTAL);
...
public:
    ...
    struct mob_twilight_eggsAI : public Scripted_NoMovementAI
    {
        ...
        void Reset()
        {
			...
			if (Creature* pTenebron = me->FindNearestCreature(NPC_TENEBRON, 100.0f))
				me->DisappearAndDie();
        }

        void SpawnWhelps()
        {
			if(instance->GetData(TYPE_TENEBRON_PREKILLED) && instance->GetData(TYPE_SARTHARION_EVENT) != IN_PROGRESS)
				me->DisappearAndDie();
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (m_uiHatchEggTimer <= uiDiff)
            {
                ...
                if (Tenebron)
                    (CAST_AI(mob_tenebron::mob_tenebronAI, Tenebron->AI()))->DoAction(ACTION_CLOSE_PORTAL);
                SpawnWhelps();
            }
            else
                m_uiHatchEggTimer -= uiDiff;
    ...
    struct npc_flame_tsunamiAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (Tsunami_Timer <= diff)
            {
				...
				//DoCast(SPELL_FLAME_TSUNAMI_LEAP);
                Tsunami_Timer = 500;
