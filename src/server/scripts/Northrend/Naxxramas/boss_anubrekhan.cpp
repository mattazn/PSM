...
public:
    ...
    struct boss_anubrekhanAI : public BossAI
    {
        ...
        void EnterCombat(Unit* /*who*/)
        {
			...
			if (GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
			{
				me->CallForHelp(5.0f);  //Adds should pull with the boss
			}
            Talk(SAY_AGGRO);
            events.ScheduleEvent(EVENT_IMPALE, urand(10000, 20000));
            events.ScheduleEvent(EVENT_LOCUST, 80000);
            events.ScheduleEvent(EVENT_BERSERK, 600000);
			events.ScheduleEvent(EVENT_CHECKPLAYERS, 15000);
            if (GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
                events.ScheduleEvent(EVENT_SPAWN_GUARDIAN_NORMAL, 20000);
        }

        void MoveInLineOfSight(Unit* who)
        {
            if (!hasTaunted && me->IsWithinDistInMap(who, 110.0f) && who->GetTypeId() == TYPEID_PLAYER)
            {
                Talk(SAY_GREET);
                hasTaunted = true;
            }
		...
		void ScarabSummon()
		{
			...
			for (Map::PlayerList::const_iterator itr = PlayerList.begin(); itr != PlayerList.end(); ++itr)
				if (Player* player = itr->getSource())
					if (!player->isAlive())
						if(urand(0,2))
							player->CastSpell(player, SPELL_SUMMON_CORPSE_SCARABS_PLR, true, NULL, NULL, me->GetGUID());
		}
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_LOCUST:
                        ...
                        events.ScheduleEvent(EVENT_LOCUST, urand(80000, 100000));
                        break;
					case EVENT_CHECKPLAYERS:
						ScarabSummon();
						events.ScheduleEvent(EVENT_CHECKPLAYERS, urand(15000, 50000));
						break;
