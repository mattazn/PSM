...
public:
    ...
    struct boss_four_horsemenAI : public BossAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
                ...
                switch (eventId)
                {
                    case EVENT_MARK:
						...
						if(caster)
							...
							events.ScheduleEvent(EVENT_MARK, 12000);
                        break;
                    case EVENT_CAST:
                        if (!(rand()%5))
            ...
            else if ((!DoSpellAttackIfReady(SPELL_SECONDARY(id)) || !me->IsWithinLOSInMap(me->getVictim())) && movementCompleted && !doDelayPunish && !me->CanReachWithMeleeAttack(me->getVictim()))
                doDelayPunish = true;
        }
    };
