...
static float M_PI_F = float(3.14159265358979323846);
...
public:
    ...
    struct boss_kelthuzadAI : public BossAI
    {
        ...
        void Reset()
        {
			...
			if((me->GetPositionX() == 3032.00f) && (me->GetPositionY() == -3461.00f) && (me->GetPositionZ() == 298.00f))
				me->DisappearAndDie();
			if((me->GetPositionX() == 2909.809f) && (me->GetPositionY() == -3960.355f) && (me->GetPositionZ() == 273.577f))
				me->DisappearAndDie();
			if((me->GetPositionX() == 2517.762f) && (me->GetPositionY() == -2952.419f) && (me->GetPositionZ() == 245.551f))
				me->DisappearAndDie();
			if((me->GetPositionX() == 3444.301f) && (me->GetPositionY() == -2996.159f) && (me->GetPositionZ() == 295.942f))
				me->DisappearAndDie();
			if((me->GetPositionX() == 3509.868f) && (me->GetPositionY() == -3894.069f) && (me->GetPositionZ() == 294.114f))
				me->DisappearAndDie();
		...
		void SummonMob(uint32 uiType)
		{
			...
			float fX, fY, fZ;
			fX = instance->GetData(DATA_CENTER_X);
			fY = instance->GetData(DATA_CENTER_Y);
			fZ = instance->GetData(DATA_CENTER_Z);

			fX += M_F_RANGE * cos(fAngle);
			fY += M_F_RANGE * sin(fAngle);
			fZ += M_F_HEIGHT;

			Trinity::NormalizeMapCoord(fX);
			Trinity::NormalizeMapCoord(fY);

			me->SummonCreature(uiType, fX, fY, fZ, 0.0f, TEMPSUMMON_CORPSE_DESPAWN, 5000);
		}
		...
		void JustSummoned(Creature* pSummoned)
		{
			switch (pSummoned->GetEntry())
			{
				...
				case NPC_WEAVER:
				{
						if (instance)
						{
							pSummoned->GetMotionMaster()->MovePoint(0, 3715.952f, -5106.707f, 141.289f, true);
						}
					break;
				}
			}
                ...
                while (uint32 eventId = events.GetEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_PHASE:
                            ...
                            events.ScheduleEvent(EVENT_BLAST, urand(45000, 60000)); //Cooldown of 45 seconds
                ...
                if (uint32 eventId = events.GetEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CHAIN:
                        {
                            ...
                            events.RepeatEvent(90000); //Cooldown of 90 seconds
                            break;
                        }
