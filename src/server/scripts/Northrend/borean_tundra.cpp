...
public:
    ...
    {
        mob_nerubar_victimAI(Creature* pCreature) : ScriptedAI(pCreature) { }
        ...
        void SpellHit(Unit* caster, const SpellInfo* spell)
        {
            ...
            if (caster->GetTypeId() == TYPEID_PLAYER && spell->Id == SPELL_EMERGENCY_TORCH)
            {
                if (GameObject* go = me->FindNearestGameObject(GO_TEMP_CRASHED_FLYING_MACHINE,10.0f))
                {
                    switch(me->GetEntry())
                    {
                        case EASTERN_WRECK_KC: caster->ToPlayer()->KilledMonsterCredit(EASTERN_WRECK_KC,0); break;
                        case SOUTHERN_WRECK_KC: caster->ToPlayer()->KilledMonsterCredit(SOUTHERN_WRECK_KC,0); break;
                        case NORTHWESTERN_WRECK_KC: caster->ToPlayer()->KilledMonsterCredit(NORTHWESTERN_WRECK_KC,0); break;
                    }
                }
            }
        }
...
class item_snq_control_unit : public ItemScript
{
    public:
        ...
        bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
        {
			...
			if (Creature* kryx = player->FindNearestCreature(25629, 30.0f))
				return false;
			else
			{
					if(Unit* sinkholeOne = player->FindNearestCreature(25664, 50.0f))
