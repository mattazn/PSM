...
public:
    ...
    struct boss_magus_telestraAI : public ScriptedAI
    {
        ...
        {
			if(instance)
				if(instance->GetData(DATA_MAGUS_TELESTRA_EVENT) != IN_PROGRESS)
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if ((Phase == 1) ||(Phase == 3))
            {
                if (bFireMagusDead && bFrostMagusDead && bArcaneMagusDead)
                {
					...
					uiIceNovaTimer = uiIceNovaTimer + 4*IN_MILLISECONDS;
					uiGravityWellTimer= uiGravityWellTimer + 4*IN_MILLISECONDS;
					uiFireBombTimer = uiFireBombTimer + 4*IN_MILLISECONDS;
