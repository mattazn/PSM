...
enum RiftSpells
{
	...
	SPELL_SPARKMODEL							  = 47687,
	SPELL_SUMMON_MANA_WRAITH					  = 47692,
};
enum WraithSpells
{
    ...
    public:
        ...
        struct mob_chaotic_riftAI : public Scripted_NoMovementAI
        {
            ...
            void Reset()
            {
				...
				if(!me->HasAura(SPELL_SPARKMODEL))
				DoCast(me, SPELL_SPARKMODEL, false);
            }
...
class mob_crazed_mana_wraith : public CreatureScript
{
    public:
        mob_crazed_mana_wraith() : CreatureScript("mob_crazed_mana_wraith") { }

        struct mob_crazed_mana_wraithAI : public ScriptedAI
        {
            mob_crazed_mana_wraithAI(Creature* creature) : ScriptedAI(creature)
            {
                instance = me->GetInstanceScript();
            }
            ...
            void Reset()
            {
				ArcaneMissiles_Timer = 3000+rand()%1000;
            }
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new mob_crazed_mana_wraithAI(creature);
        }
};
