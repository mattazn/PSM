...
class mob_land_mine_bunny : public CreatureScript
{
public:
    mob_land_mine_bunny() : CreatureScript("mob_land_mine_bunny") { }

    CreatureAI* GetAI(Creature* creature) const
    {
        return new mob_land_mine_bunnyAI (creature);
    }

    struct mob_land_mine_bunnyAI : public ScriptedAI
    {
        mob_land_mine_bunnyAI(Creature* creature) : ScriptedAI(creature)
		{
        }
...
class spell_q12915_hurl_boulder : public SpellScriptLoader
{
    public:
        spell_q12915_hurl_boulder() : SpellScriptLoader("spell_q12915_hurl_boulder") { }

        class spell_q12915_hurl_boulder_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_q12915_hurl_boulder_SpellScript)

            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                if (Unit* caster = GetCaster())
                    if (Player* player = caster->ToPlayer())
                        if (player->GetQuestStatus(QUEST_MENDING_FENCES) == QUEST_STATUS_INCOMPLETE)
                            for (uint32 i = 0; i < urand(3, 5); ++i)
                                player->CastSpell(player, SPELL_SUMMON_EARTHEN, false);
            }

            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_q12915_hurl_boulder_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

    SpellScript* GetSpellScript() const
    {
        return new spell_q12915_hurl_boulder_SpellScript();
    }
};
...
#define GOSSIP_MAX    "I'm interested."

enum eHonestMax
{
    SPELL_CREATE_ALLIANCE	= 60126,
	SPELL_CREATE_HORDE		= 60128,
	SPELL_COLD_WEATHER		= 54197
};
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        if (!player->HasSpell(SPELL_COLD_WEATHER))
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_MAX, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
            player->PlayerTalkClass->SendGossipMenu(14088, creature->GetGUID());
        }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF+1:
			if(player->GetTeamId() == TEAM_ALLIANCE)
				player->CastSpell(player, SPELL_CREATE_ALLIANCE);
			else
				player->CastSpell(player, SPELL_CREATE_HORDE);
            break;
        }
...
enum FateOfTheTitans
{
    QUEST_FATE_OF_THE_TITANS    = 12986,

    NPC_MOBILE_DATABANK         = 30313,

    AREA_TEMPLE_OF_INVENTION    = 4466,
    AREA_TEMPLE_OF_WINTER       = 4465,
    AREA_TEMPLE_OF_LIFE         = 4463,
    AREA_TEMPLE_OF_ORDER        = 4464,

    NPC_TEMPLE_OF_INVENTION    = 30315,
    NPC_TEMPLE_OF_WINTER       = 30317,
    NPC_TEMPLE_OF_LIFE         = 30316,
    NPC_TEMPLE_OF_ORDER        = 30318,
};

class npc_mobile_databank : public CreatureScript
{
    public:
        npc_mobile_databank() : CreatureScript("npc_mobile_databank") { }

        struct npc_mobile_databankAI : public ScriptedAI
        {
            npc_mobile_databankAI(Creature* creature) : ScriptedAI(creature) { }
            ...
            void UpdateAI(uint32 const diff)
            {
                if (CheckTimer <= diff)
                {
                    ...
                    {
                        if (player->GetQuestStatus(QUEST_FATE_OF_THE_TITANS) == QUEST_STATUS_INCOMPLETE)
                        {
                            switch (me->GetAreaId())
                            {
                                case AREA_TEMPLE_OF_INVENTION:
                                    player->KilledMonsterCredit(NPC_TEMPLE_OF_INVENTION, 0);
                                    break;
                                case AREA_TEMPLE_OF_WINTER:
                                    player->KilledMonsterCredit(NPC_TEMPLE_OF_WINTER, 0);
                                    break;
                                case AREA_TEMPLE_OF_LIFE:
                                    player->KilledMonsterCredit(NPC_TEMPLE_OF_LIFE, 0);
                                    break;
                                case AREA_TEMPLE_OF_ORDER:
                                    player->KilledMonsterCredit(NPC_TEMPLE_OF_ORDER, 0);
                                    break;
                                default:
                                    break;
                            }
        ...
        CreatureAI* GetAI(Creature* creature) const
        {
            return new npc_mobile_databankAI(creature);
        }
};

enum ForgingTheKeystone
{
    QUEST_FORGING_THE_KEYSTONE  = 13285,
};

class npc_brann_forging_the_key : public CreatureScript
{
    public:
        npc_brann_forging_the_key() : CreatureScript("npc_brann_forging_the_key") { }

        bool OnGossipHello(Player* player, Creature* creature)
        {
            if (player->GetQuestStatus(QUEST_FORGING_THE_KEYSTONE) == QUEST_STATUS_INCOMPLETE)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "I'm ready, Brann. Let's make the keystone.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
enum BrannsCommunicator
{
    ...
    QUEST_ON_BRANNS_TAIL        = 12854,
    QUEST_MISSING_BRONZEBEARD   = 12895,
};

class npcs_branns_communicator_retrival : public CreatureScript
{
    public:
        npcs_branns_communicator_retrival() : CreatureScript("npcs_branns_communicator_retrival") { }

        bool OnGossipHello(Player* player, Creature* creature)
        {
            if (!player->HasItemCount(ITEM_BRANNS_COMMUNICATOR, 1, true))
                if (player->GetQuestStatus(QUEST_ON_BRANNS_TAIL) == QUEST_STATUS_REWARDED || player->GetQuestStatus(QUEST_MISSING_BRONZEBEARD) == QUEST_STATUS_REWARDED)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "I've lost my Brann's Communicator.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
void AddSC_storm_peaks()
{
	...
	new mob_land_mine_bunny();
	new spell_gen_landmine_knockback_achievement_aura();
	new achievement_mine_sweeper();
	new spell_q12915_hurl_boulder();
	new npc_honest_max();
    new npc_mobile_databank();
    new npc_brann_forging_the_key();
    new npcs_branns_communicator_retrival();
}
