    ...
    public:
        ...
        struct boss_elder_nadoxAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                {
					if (Creature* pEgg = me->FindNearestCreature(30172, 100, true))
					{
						pEgg->CastSpell(pEgg, SPELL_SUMMON_SWARMERS);
						pEgg->CastSpell(pEgg, SPELL_EGG_HATCH);
						pEgg->DisappearAndDie();
					}
                ...
                if (uiGuardSpawnTimer <= diff)
                {
                    me->MonsterTextEmote(EMOTE_HATCHES, me->GetGUID(), true);
					if (Creature* pEgg = me->FindNearestCreature(30173, 100, true))
					{
						pEgg->CastSpell(pEgg, SPELL_SUMMON_SWARM_GUARD);
						pEgg->CastSpell(pEgg, SPELL_EGG_HATCH);
						pEgg->DisappearAndDie();
					}
                    uiGuardSpawnTimer = 25000;
