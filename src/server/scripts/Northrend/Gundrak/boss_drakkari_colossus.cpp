    ...
    public:
        ...
        struct boss_drakkari_colossusAI : public BossAI
        {
            boss_drakkari_colossusAI(Creature* creature) : BossAI(creature, DATA_DRAKKARI_COLOSSUS_EVENT)
            {
                ...
                introDone = false;
				me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE|UNIT_FLAG_NOT_SELECTABLE|UNIT_FLAG_NOT_ATTACKABLE_1|UNIT_FLAG_IMMUNE_TO_PC|UNIT_FLAG_DISABLE_MOVE);
            ...
            void Reset()
            {
                ...
                if (GetData(DATA_INTRO_DONE))
                {
                    ...
                    me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE|UNIT_FLAG_NOT_SELECTABLE|UNIT_FLAG_NOT_ATTACKABLE_1|UNIT_FLAG_IMMUNE_TO_PC|UNIT_FLAG_DISABLE_MOVE);
            ...
            void DoAction(const int32 action)
            {
                switch (action)
                {
                    ...
                    case ACTION_FREEZE_COLOSSUS:
						...
						me->RemoveAllAuras();
                        me->SetReactState(REACT_PASSIVE);
						me->ApplySpellImmune(1, IMMUNITY_MECHANIC, MECHANIC_STUN, false);
						me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE|UNIT_FLAG_NOT_SELECTABLE|UNIT_FLAG_NOT_ATTACKABLE_1|UNIT_FLAG_IMMUNE_TO_PC|UNIT_FLAG_DISABLE_MOVE);
                        DoCast(me, SPELL_FREEZE_ANIM);
            ...
            {
                if (me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE|UNIT_FLAG_NOT_SELECTABLE|UNIT_FLAG_NOT_ATTACKABLE_1|UNIT_FLAG_IMMUNE_TO_PC|UNIT_FLAG_DISABLE_MOVE))
                    damage = 0;
    ...
    public:
        ...
        struct boss_drakkari_elementalAI : public ScriptedAI
        {
            ...
            void Reset()
            {
				...
				events.ScheduleEvent(EVENT_PUDDLE, 7*IN_MILLISECONDS);
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
						...
						case EVENT_PUDDLE:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true))
								DoCast(me->getVictim(), SPELL_PUDDLE);
                            events.ScheduleEvent(EVENT_PUDDLE, urand(15000, 19000));
                    }
                }
