...
enum StormcrowAmulet
{
    DISPLAYID              = 17447
};

class spell_stormcrow_amulet : public SpellScriptLoader
{
    public:
        spell_stormcrow_amulet() : SpellScriptLoader("spell_stormcrow_amulet") { }

        class spell_stormcrow_amulet_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_stormcrow_amulet_AuraScript);

            void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* target = GetTarget())
					target->SetUInt32Value(UNIT_FIELD_DISPLAYID, DISPLAYID);
            }

            void Register()
            {
                AfterEffectRemove += AuraEffectRemoveFn(spell_stormcrow_amulet_AuraScript::OnRemove, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        ...
        AuraScript* GetAuraScript() const
        {
            return new spell_stormcrow_amulet_AuraScript();
        }
};
