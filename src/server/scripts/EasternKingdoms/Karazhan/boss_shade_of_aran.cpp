        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (NormalCastTimer <= diff)
            {
                if (!me->IsNonMeleeSpellCasted(false))
                {
                    ...
                    if (AvailableSpells)
                    {
						...
						me->ApplySpellImmune(1, IMMUNITY_MECHANIC, MECHANIC_INTERRUPT, false); 
            ...
            if (SecondarySpellTimer <= diff)
            {
                switch (rand()%3)
                {
                    ...
                    case 2:
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
                            DoCast(target, SPELL_DRAGONSBREATH);
                        break;
                }
            ...
            if (SuperCastTimer <= diff)
            {
                ...
                switch (LastSuperSpell)
                {
                    case SUPER_AE:
						...
						me->ApplySpellImmune(1, IMMUNITY_MECHANIC, MECHANIC_INTERRUPT, true);   
            ...
            if (!ElementalsSpawned && HealthBelowPct(40))
            {
                ...
                for (uint32 i = 0; i < 4; ++i)
                {
                    ...
                    if (Creature* unit = me->SummonCreature(CREATURE_WATER_ELEMENTAL, xm, ym, 0.0f, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 90000))
                    {
