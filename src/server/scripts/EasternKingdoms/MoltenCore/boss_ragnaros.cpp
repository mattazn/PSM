    ...
    public:
        ...
        struct boss_ragnarosAI : public BossAI
        {
            ...
            void EnterCombat(Unit* victim)
            {
				...
				events.ScheduleEvent(EVENT_MELT, 10000);
            }
                    ...
                    while (uint32 eventId = events.ExecuteEvent())
                    {
                        switch (eventId)
                        {
							...
							case EVENT_MELT:
								...
								{
									case 1:
										if(Unit* target = me->SelectNearbyTarget(0, me->GetMeleeReach()))
											DoCast(target, SPELL_MELT_WEAPON);
										break;
									default:
										break;
								}
...
class mob_son_of_flame : public CreatureScript
{
    public:
        ...
        struct mob_son_of_flameAI : public ScriptedAI //didnt work correctly in EAI for me...
        {
			...
			void Reset()
			{
				Pyroblast_Timer = urand(2500, 5000);
				DoCast(SPELL_LAVA_SHIELD_AURA);
			}
            ...
            void UpdateAI(const uint32 diff)
            {
				...
				{
					DoCast(me->getVictim(), SPELL_PYRO_BLAST);
					Pyroblast_Timer = urand(2500, 5000);
				}
				else
