...
public:
    ...
    static bool HandleGMTicketGetByIdCommand(ChatHandler* handler, char const* args)
    {
        ...
        ticket->SetViewed();
        ticket->SaveToDB(trans);
		HandleGMTicketAssignTo(handler, ticketId, handler->GetSession()->GetPlayer()->GetName().c_str());
    ...
    static bool HandleGMTicketAssignTo(ChatHandler* handler, uint32 ticketId, char const* gmName)
    {
        char const* targetStr = gmName;
        if (!targetStr)
            return false;
