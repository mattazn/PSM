...
public:
	...
	if(chr->getLevel() <= 57)
		MAX_XP = 25;
	if(chr->getLevel() >= 58 && chr->getLevel() <= 67)
		MAX_XP = 15;
	if(chr->getLevel() >= 68)
		MAX_XP = 5;
	if(chr->getLevel() == 80)
	{
		handler->PSendSysMessage("You cannot change your experience rates at level 80.");
		return false;
	}
    ...
    if (xp_rate < 0 || xp_rate > MAX_XP)
    {
       handler->PSendSysMessage("Please choose a value from 0 to %i.", MAX_XP);
       handler->SetSentErrorMessage(true);
        return false;
    }

	if(chr->GetXpRate() <= 1 && !chr->IsRFsDisabled() && xp_rate > 1)
		disableAchi = true;
	else
		disableAchi = false;
	...
	if(xp_rate == 0)
		handler->PSendSysMessage("You have disabled experience gain. To re-enable it set your XP rate to atleast 1.");
	else
		handler->PSendSysMessage("Your XP rate is now: %i.", chr->GetXpRate());

	if(disableAchi)
	{
		chr->SetRFsDisabled(true);
		handler->PSendSysMessage("You are no longer eligible for realm first achievements.");
	}
